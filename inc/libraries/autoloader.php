 ...
 *
 * The argument should be in the form: Codeable_Users_Table_Plugin\Namespace. The
 ...
 *
 * @since      1.0.0
 * @package    Codeable_User_Table/libraries
...
spl_autoload_register(
	function( $class_name ) {
		...
		if ( false === strpos( $class_name, 'Codeable_Users_Table_Plugin' ) ) {
			return;
		}
		...
		if ( file_exists( $filepath ) ) {
			...
			wp_die(
				sprintf(
					/* translators: %s: File Path tag */
					esc_html__( 'The file attempting to be loaded at %s does not exist.', NS\CODEABLE_USERS_TABLE_TEXT_DOMAIN ),
					esc_attr( $filepath )
				)
			);
		}
